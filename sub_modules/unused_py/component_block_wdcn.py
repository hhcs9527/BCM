...
def get_DCN_norm_down_layer(norm_type, channel):
    if norm_type == 'batch':
        norm_layer = nn.Sequential(
            DCN(channel, channel, kernel_size=3, padding=1, stride=1),
            nn.BatchNorm2d(channel),
            nn.LeakyReLU(0.2, True),
            DCN(channel, channel//2, kernel_size=3, padding=1, stride=1))
    elif norm_type == 'instance':
        norm_layer = nn.Sequential(
            DCN(channel, channel, kernel_size=3, padding=1, stride=1),
...
def get_DCN_norm_layer(norm_type, channel):
    if norm_type == 'batch':
        norm_layer = nn.Sequential(
            DCN(channel, channel, kernel_size=3, padding=1, stride=1),
            nn.BatchNorm2d(channel),
...
class Res_DCN_Block(nn.Module):
    def __init__(self, norm, channels):
        super().__init__()
        self.res_block = get_DCN_norm_layer(norm, channels)
...
class Guide_DCN_FE_Block(nn.Module):
    def __init__(self, opt, in_c, out_c, stride_len, kernel_size = 5):
        super().__init__()
        self.LeakyReLU = nn.LeakyReLU(0.2, True)
        self.guide_DCN = nn.Sequential(
            #StripPooling(opt, in_c, strip_size = 1),
            DCN(in_c, out_c, kernel_size=(kernel_size, kernel_size), stride=1, padding = kernel_size//2),
            #nn.Conv2d(in_c, out_c, kernel_size = kernel_size, padding=kernel_size//2),
            #get_DCN_norm_layer('leakyrelu', out_c),
        ...
        self.Residual_DCN = nn.Sequential(
                        self.Residual_lv1, 
                        self.Residual_lv2,
                        )

    def forward(self, x):
        return self.Residual_DCN(self.guide_DCN(x))

class Guide_DCN_Block(nn.Module):
    def __init__(self, opt, in_c, out_c, stride_len, kernel_size = 5):
        super().__init__()
        self.LeakyReLU = nn.LeakyReLU(0.2, True)
        self.guide_DCN = nn.Sequential(
...
class Guide_Block_FPN_downsample(nn.Module):
    def __init__(self, in_c, out_c, stride_len):
        super().__init__()
        self.guide_DCN = nn.Sequential(
            DCN(in_c, out_c, kernel_size=(3,3), stride=stride_len, padding=1),
            nn.LeakyReLU(0.2, True),
            nn.Conv2d(out_c, out_c, kernel_size=3, padding=1))

    def forward(self, x):
        return self.guide_DCN(x)
