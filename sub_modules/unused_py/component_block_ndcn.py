...
class Guide_DCN_Block(nn.Module):
    def __init__(self, in_c, out_c, stride_len, kernel_size = 3):
        super().__init__()
        self.LeakyReLU = nn.LeakyReLU(0.2, True)
        self.guide_DCN = nn.Sequential(
            nn.Conv2d(in_c, out_c, kernel_size=(kernel_size, kernel_size), stride=1, padding = kernel_size//2),
    ...
    def Residual_DCN(self, x):
        first_order = x 
        second_order = self.Residual_lv1(first_order) + first_order
        return self.Residual_lv2(second_order) + first_order + second_order
...
class Guide_Block_FPN_downsample(nn.Module):
    def __init__(self, in_c, out_c, stride_len):
        super().__init__()
        self.guide_DCN = nn.Sequential(
            nn.Conv2d(in_c, out_c, kernel_size=(3,3), stride=stride_len, padding=1),
            nn.LeakyReLU(0.2, True),
