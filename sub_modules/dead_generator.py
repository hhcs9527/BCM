...
class DMPHN_1_2_4_BRR_Both(nn.Module):
    def __init__(self, opt):
        super().__init__()
        compression_ratio = 1
        self.encoder_lv3 = Encoder(opt)#BRR_Encoder(opt, level = 3) 
        self.encoder_lv2 = BRR_Encoder(opt, level = 2) 
        self.encoder_lv1 = BRR_Encoder(opt, level = 1) 

        self.decoder_lv3 = Decoder(opt)#BRR_Decoder(opt)
        self.decoder_lv2 = BRR_Decoder(opt)   
        self.decoder_lv1 = BRR_Decoder(opt)
    ...
    def forward(self, input):
        ...
        residual_lv3_bot = self.decoder_lv3(feature_lv3_bot_dict)
        '''
        residual_lv3_top['reblur_filter'] = torch.cat((residual_lv3_top['reblur_filter']), dim = 1) 
        residual_lv3_bot['reblur_filter'] = torch.cat((residual_lv3_bot['reblur_filter']), dim = 1)      
        residual_lv3_reblur_filter = torch.cat((residual_lv3_top['reblur_filter'], residual_lv3_bot['reblur_filter']), 2)  
        split = list(torch.split(residual_lv3_reblur_filter, self.num_split, dim = 1))    
        self.reblur_filters += split      
        residual_lv3_top = residual_lv3_top['decode_feature']
        residual_lv3_bot = residual_lv3_bot['decode_feature']
        '''
        ...
        residual_lv2 = self.decoder_lv2(feature_lv2_dict)
        #'''
        residual_lv2['reblur_filter'] = torch.cat((residual_lv2['reblur_filter']), dim = 1)
        split = list(torch.split(residual_lv2['reblur_filter'], self.num_split, dim = 1))
        self.reblur_filters += split 
        residual_lv2 = residual_lv2['decode_feature']
        #'''
        ...
        deblur_image = self.decoder_lv1(feature_lv1_dict)   
        deblur_image['reblur_filter'] = torch.cat((deblur_image['reblur_filter']), dim = 1)
        split = list(torch.split(deblur_image['reblur_filter'], self.num_split, dim = 1))
        self.reblur_filters += split 
        deblur_image = deblur_image['decode_feature']
        
        return {'deblur' : deblur_image, 'reblur_filter' : self.reblur_filters}

class DMPHN_1_2_4_BR(nn.Module):
    def __init__(self, opt):
        super().__init__()
        compression_ratio = 1
        self.encoder_lv3 = Encoder(opt) 
        self.encoder_lv2 = Encoder(opt) 
        self.encoder_lv1 = BR_Encoder(opt, level = 1) 

        self.decoder_lv3 = Decoder(opt)
        self.decoder_lv2 = Decoder(opt)   
        self.decoder_lv1 = Decoder(opt)
        self.num_split = opt.per_pix_kernel ** 2
...
class DMPHN_RFDB_reblur_1_2_4(nn.Module):
    def __init__(self, opt):
        super().__init__()
        compression_ratio = 1
        self.encoder_lv3 = RFDB_Encoder(opt, level = 3) 
        self.encoder_lv2 = RFDB_Encoder(opt, level = 2) 
        self.encoder_lv1 = RFDB_Encoder(opt, level = 1) 

        self.decoder_lv3 = Decoder_triple_reblur_front(opt)
        self.decoder_lv2 = Decoder_triple_reblur_front(opt)   
        self.decoder_lv1 = Decoder_triple_reblur_front(opt)
...
class DMPHN_1_2_4_BER(nn.Module):
    def __init__(self, opt):
        super().__init__()
        compression_ratio = 1
        self.encoder_lv3 = Encoder(opt) 
        self.encoder_lv2 = Encoder(opt) 
        self.encoder_lv1 = BER_Encoder(opt, level = 1) 
...
class DMPHN_1_2_4_BRRM(nn.Module):
    def __init__(self, opt):
        super().__init__()
        compression_ratio = 1
        self.encoder_lv3 = Encoder(opt) 
        self.encoder_lv2 = Encoder(opt) 
        self.encoder_lv1 = BRRM_Encoder(opt, level = 1) 
    ...
    def forward(self, input):
        ...
        for key in feature_lv1_1.keys():
            if key == 'encode_feature':
                ...
                self.blur_mask += [feature_lv1_1[key]]
        ...
        return {'deblur' : deblur_image, 'reblur_filter' : self.reblur_filters, 'blurmask': self.blur_mask}
