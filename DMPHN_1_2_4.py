...
def main():
    ...
    deblur_model = DMPHN_1_2_4().cuda(GPU)      
    ...
    for epoch in range(args.start_epoch, EPOCHS):
        ...
        for iteration, images in enumerate(train_dataloader):            
            ...
            deblur = deblur_model(images_lv1)
            loss_lv1 = mse(deblur, gt)
            ...
            sub_psnr = sub_psnr + PSNR(deblur, gt)
            sub_ssim = sub_ssim + SSIM(deblur, gt)
        ...
        if (epoch + 1) % eval_per_epoch == 0:
            ...
            for iteration, images in enumerate(test_dataloader):
                with torch.no_grad():      
                    ...
                    print(PSNR(deblur_dict['deblur'], gt), SSIM(deblur_dict['deblur'], gt))
