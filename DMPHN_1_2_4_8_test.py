...
def main():
    ...
    sample_list = sorted(glob.glob('/home/hh/Desktop/GoPro' + '/test' + '/**/blur/*.png', recursive=True))
    iteration = 0.0
    test_time = 0.0
    for images_name in sample_list:#os.listdir(SAMPLE_DIR):
        #print(images_name)
    	with torch.no_grad():
            images_lv1 = transforms.ToTensor()(Image.open(images_name).convert('RGB'))#SAMPLE_DIR + '/' + images_name
            ...
            save_images(deblur_image.data + 0.5, f'{iteration}_deblur.png') 
            iteration += 1
