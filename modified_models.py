...
class Encoder(nn.Module):
    def __init__(self):
        ...
        #Conv1
        self.layer1 = PacConv2d(3, 32, kernel_size=3, padding=1)
        ...
        #Conv2
        self.layer5 = PacConv2d(32, 64, kernel_size=3, stride=2, padding=1)
        ...
        #Conv3
        self.layer9 = PacConv2d(64, 128, kernel_size=3, stride=2, padding=1)
...
class Decoder(nn.Module):
    def __init__(self):
        ...
        self.layer16 = PacConvTranspose2d(128, 64, kernel_size=3, output_padding = 1, stride=2, padding=1)
        #Deconv2
        ...
        self.layer20 = PacConvTranspose2d(64, 32, kernel_size=3, output_padding = 1, stride=2, padding=1)
        #Deconv1
    ...
    def forward(self,x, ef_lv2, ef_lv1):
        ...
        x = self.layer13(x) + x
        x = self.layer14(x) + x
        x = self.layer16(x, ef_lv2)
        #x = self.layer16(x)
             
        #Deconv2
        x = self.layer17(x) + x
        x = self.layer18(x) + x
        x = self.layer20(x, ef_lv1)
        #x = self.layer20(x)
        #Deconv1
        x = self.layer21(x) + x
        x = self.layer22(x) + x
